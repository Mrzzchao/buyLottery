<template v-if="bf">
    <div class="l-flex-1 l-relative l-flex-column">
        <ul class="dang-list-ul l-full l-scroll-y wrapper">
            <li class="dang-list-li" v-for="item in bf" :class="{'curlight':item.rec}">
                <expand :match="item">
...
export default {
    ...
    computed: {
        bf() {
            return this.$store.state.jczqHome.play.filter.bf
        },
        bfFmt() {
            let obj = {}
            this.bf.forEach((match) => {
                obj[match.mid] = match
            })
            return obj
        },
    ...
    destroyed() {
        this.$store.commit(mTypes.resetFilter,'bf')
    },
    filters: {
        selectedFormat(selection, num) {
            if(!selection) return '点击展开投注选项'
            let str = ''
            selection.bf.forEach((type) => {
                str += SelectNameMap.bf[type]
                str += ';'
            })
            return str
        },
